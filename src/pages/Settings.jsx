  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        ...
        {testResult && (
          <p className={`text-xs mt-1 ${testResult.ok ? 'text-green-600' : 'text-red-600'}`}>{testResult.msg}</p>
        )}
      </div>

      {/* OpenAI Key (Whisper voice) */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <h3 className="text-sm font-semibold text-gray-800 mb-1">🎙 OpenAI API Key <span className="text-xs font-normal text-gray-400">(Whisper voice transcription)</span></h3>
        <p className="text-xs text-gray-500 mb-4">
          Powers multilingual voice input — Chinese, English, Spanish auto-detected with no language selection needed.
          Get your key at <a href="https://platform.openai.com/api-keys" target="_blank" rel="noreferrer" className="text-blue-600 underline">platform.openai.com</a> → API Keys.
          Cost: ~$0.006 / minute of audio.
        </p>
        {openaiMasked && (
          <div className="flex items-center gap-2 mb-2 px-3 py-2 bg-green-50 border border-green-200 rounded-lg">
            <span className="text-sm text-green-700">✓ OpenAI key saved</span>
            <button onClick={() => { setOpenaiMasked(false); clearOpenAIKeyCache() }} className="ml-auto text-xs text-blue-600 hover:underline">Replace key</button>
          </div>
        )}
        {!openaiMasked && (
          <div className="flex gap-2 mb-2">
        ...
        <button onClick={handleTestOpenAIKey} disabled={openaiTestLoad} className="px-4 py-2 border border-gray-300 hover:bg-gray-50 text-gray-700 text-sm rounded-lg">
      ...
      {/* Name Mappings */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <h3 className="text-sm font-semibold text-gray-800 mb-1">👥 Employee Name Mappings</h3>
        <p className="text-xs text-gray-500 mb-3">
          Map informal names from DingTalk notes to full employee names.<br/>
          Format: one mapping per line — <code className="bg-gray-100 px-1 rounded">informal name = Full Name</code>
        </p>
        <textarea
          value={nameMap}
        ...
        />
        <p className="text-xs text-gray-400 mt-1">These will be used to match names in meeting notes to actual employees.</p>
      </div>

      {/* Vendor Mappings */}
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
        <h3 className="text-sm font-semibold text-gray-800 mb-1">🏢 Sublet Vendor Mappings</h3>
        <p className="text-xs text-gray-500 mb-3">
          Map vendor names from meeting notes to their full company names.<br/>
          Format: <code className="bg-gray-100 px-1 rounded">short name = Full Company Name</code>
        </p>
        <textarea
          value={vendorMap}
